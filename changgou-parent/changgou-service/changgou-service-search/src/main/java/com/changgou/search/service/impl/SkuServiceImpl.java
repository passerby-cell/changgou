 ...
 *//*
    ...
    public Map search(Map<String, String> searchMap) {
        ...
        if(!StringUtils.isEmpty(sortField) && !StringUtils.isEmpty(sortRule)) {
            //执行排序
            nativeSearchQueryBuilder.withSort(SortBuilders.fieldSort(sortField).order(sortRule.equalsIgnoreCase("ASC")? javax.swing.SortOrder.ASC: SortOrder.DESC));
...
public class SkuServiceImpl implements SkuService {
    ...
    public void importData() {
        //1.调用 goods微服务的fegin 查询 符合条件的sku的数据
//        Result<List<Sku>> skuResult = skuFeign.findByStatus("1");
        Result<PageInfo<Sku>> skuResult = skuFeign.findPage(1, 30);
        List<Sku> data = skuResult.getData().getList();//sku的列表
        //将sku的列表 转换成es中的skuinfo的列表
        List<SkuInfo> skuInfos = JSON.parseArray(JSON.toJSONString(data), SkuInfo.class);
        for (SkuInfo skuInfo : skuInfos) {
            //获取规格的数据  {"电视音响效果":"立体声","电视屏幕尺寸":"20英寸","尺码":"165"}
