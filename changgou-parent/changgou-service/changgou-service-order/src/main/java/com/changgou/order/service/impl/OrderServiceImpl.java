...
public class OrderServiceImpl implements OrderService {
    ...
    public void addOrder(Order order) {
        ...
        //获取需要下单的商品id，需要下单的商品的id信息从购物车中移除
        List<Long> skuIds = order.getSkuIds();
        for (Long l :
                skuIds) {
            OrderItem o = (OrderItem) redisTemplate.boundHashOps("cart_" + order.getUsername()).get(l);
            orderitems.add(o);
            redisTemplate.boundHashOps("cart_" + order.getUsername()).delete(l);
        }
...
//            skuFeign.decrCount(o.getSkuId(),o.getNum());
         ...
         * 订单总金额=每个商品的金额之和
         */
        order.setTotalMoney(totalprice);  //订单总金额
        order.setPayMoney(totalprice);  //订单实付金额
        //添加订单信息
